...
def train(net, optim, scheduler, names, train_dataset, epoch, args):
    ...
    for it, (feats_patch, center_patch, face_patch, coordinate_patch, np_Fs, label, mesh_paths) in enumerate(
        ...
        # save_results(pred, mask, mesh_paths)
        loss.backward()
        optim.step()
    ...
    if train.best_loss > epoch_loss:
        ...
        torch.save(best_model_wts, os.path.join('checkpoints', names, f'loss-{epoch_loss_m:.4f}-{epoch:.4f}.pkl'))

    print('epoch ({:}): {:} Train Loss: {:.4f}'.format(names, epoch, epoch_loss_m, best_epoch))
